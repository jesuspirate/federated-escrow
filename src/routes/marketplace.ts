  ...
  try {
    ...
    const orders = rows.map(order => {
      ...
      return {
        ...
        loanFullyRepaid: escrow?.loan_status === "repaid" || (escrow?.loan_repayment_id ? (() => { const rep = DB.getEscrow(escrow.loan_repayment_id); return rep && (rep.status === "COMPLETED" || rep.status === "CLAIMED"); })() : false),
      };
    });
