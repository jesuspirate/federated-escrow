...
function syncAndFormatOrder(order: OrderRow) {
  const synced = syncOrderWithEscrow(order);
  return formatOrder(synced);
}
...
router.post("/profile/:pubkey/rate", ...requireAuth, (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const existing = stmts.getRatingByOrder.get(orderId) as RatingRow | undefined;
    if (existing && existing.rater_pubkey === raterPubkey)
