...
function BrowseView({ listings, loading, pubkey, searchQuery, setSearchQuery, onSearch, onOpen, onCreate, onOrders, activeOrderCount, onNotifications, onRefresh, onSwitchToEscrow, onProfile, locale, onSwitchLocale, onChapSmart, subdomain, myFederation, onArbiters }) {
...
function ArbiterRecruitmentView({ pubkey, onBack, showToast }) {
  ...
  const handleApply = async () => {
    if (!fediProfile.trim()) return showToast("Paste your Fedi profile link", "error");
    if (!displayName.trim()) return showToast("Enter your display name", "error");
    setSubmitting(true);
    try {
      const res = await mapi("/arbiters/apply", {
        method: "POST",
        body: JSON.stringify({ fediProfile, displayName, motivation }),
      });
      if (res.error) throw new Error(res.error);
      showToast(res.message || "Application submitted!");
      setMyStatus({ status: res.status, federationDomain: res.federationDomain, displayName });
    } catch (err) { showToast(err.message, "error"); }
    setSubmitting(false);
  };
      ...
      <div style={{ padding: "0 4px 20px" }}>
        ...
        {myStatus && myStatus.status === "approved" && (
          <div style={{ padding: "14px 16px", borderRadius: 12, background: "rgba(16,185,129,0.1)", border: "2px solid rgba(16,185,129,0.3)", marginBottom: 16, textAlign: "center" }}>
            ...
            {myStatus.federationDomain && <div style={{ fontSize: 12, color: "#94a3b8", marginTop: 4 }}>Federation: {getFedName(null, myStatus.federationDomain)}</div>}
          </div>
        )}
        ...
        {(!myStatus || myStatus.status === "none" || myStatus.status === "rejected") && (
          <div style={{ padding: "16px", borderRadius: 14, background: "linear-gradient(145deg, #111827, #0f1320)", border: "1px solid #1e293b", marginBottom: 16 }}>
            ...
            <div style={{ marginBottom: 12 }}>
              <label style={{ fontSize: 12, color: "#94a3b8", fontWeight: 600, display: "block", marginBottom: 4 }}>Your Fedi Profile Link *</label>
              <input style={M.input} placeholder="@npub1...:federation.domain" value={fediProfile} onChange={e => setFediProfile(e.target.value)} />
              <div style={{ fontSize: 10, color: "#475569", marginTop: 3 }}>Open Fedi → Profile → Copy your profile link</div>
              {detectedFed && (
            ...
            <div key={a.id} style={{ padding: "12px 14px", borderRadius: 10, background: "#111827", border: "1px solid #1e293b", marginBottom: 8, display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 10, color: "#64748b", marginTop: 1 }}>
                  {a.federationDomain ? getFedName(null, a.federationDomain) : "Independent"} · Since {new Date(a.since).toLocaleDateString()}
                </div>
              </div>
